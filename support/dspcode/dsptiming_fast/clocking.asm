...
; 23Sep15 MPL after ARC suggestion for delay
...
PFOR	DC	EPFOR-PFOR-1
	DC	VIDEO+$000000+%0011000            ; Reset integ. and DC restore
...
;	DC	CLK2+P_DELAY+P1L+P2L+P3L  ; MPP - more traps in MPP mode
EPFOR
...
; CDS integrate on noise
		DC	        VIDEO+$060000+%0011011  ; Stop resetting int 6
		DC	          VIDEO+DWELL+%0001011	; Integrate noise
		DC	    	VIDEO+$000000+%0011011  ; Stop int 0
	ENDM

INTSIGNAL	MACRO
; CDS integrate on signal
		DC	        VIDEO+$060000+%0010111	; change polarity 6
		DC	          VIDEO+DWELL+%0000111	; Integrate signal
		DC	        VIDEO+$000000+%0010111  ; Stop integrate, ADC is sampling
		DC	        VIDEO+$060000+%1110111  ; start A/D conversion 6
  		DC	        VIDEO+$060000+%0010111  ; End start A/D conv. pulse 6
	ENDM
...
SXFER0	DC	ESXFER0-SXFER0-1
		...
		DC      VIDEO+$000000+%0011000                  ; Reset integrator & DC restore
...
SXFER2D	DC	ESXFER2D-SXFER2D-1
	...
	INTNOISE
		DC	CLK3+S_DELAY+RGL+S1H+S2H+S3L+SWLL+SWRL
		DC	CLK3+S_DELAY+RGL+S1H+S2H+S3L+SWLH+SWRH  ; delay - important
	INTSIGNAL
ESXFER2D
